    ...
    return (
        ...
        >
            <div className='container mx-auto px-6'>
                ...
                <p className='text-gray-400 text-center max-w-2xl mx-auto mb-16'>Skills shaped by passion, practice, and a constant love for visual storytelling.</p>
            <div className='mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-5xl'>
                {
                    skills.map((skill,index)=>(
                        <div  key={index} className='bg-dark-300 rounded-2xl p-6 hover:-translate-y-2 transition duration-300 cursor-pointer'>
                            ...
                            <div className='flex flex-wrap gap-2'>
                              {skill.tags.map((tech)=>(
                                <span key={tech} className='px-3 py-1 text-sm rounded-full bg-dark-400'>{tech}</span>
                              ))}
                            </div>
                        </div>
                    ))
